...
$(document).ready(function ()
{
    ...
    cafe_form_edit.submit(function (event)
    {
        ...
        let cafe_id = $('[data-cafe-id]').first().attr('data-cafe-id');
    ...
    form_add_drink.submit(function (event)
    {
        ...
        {
            let row = $(this);
            let weight = row.find('p').eq(0).text().replace(/[^0-9]/gi, '');
            let price = row.find('p').eq(1).text().replace(/[^0-9]/gi, '');
    ...
    $(document).on('show.bs.modal', '#modal_add_drink', function (e)
    {
        ...
        if(editMode)
        {
        ...
        else
            {
                ...
                modal.find('.modal_subtitle').text('Добавьте новый продукт в меню приложения. Сразу после добавлния продукт станет доступен для заказа.');
...
function makeAddMilk()
{
    ...
    var newRow =
        `
        <div class="row_line">
            <p class="third d-inline-block text-center mb-0">` + name + `</p>
            <p class="third third-center d-inline-block text-center mb-0">` + price + 'р'+`</p>
...
function makeAddWeight()
{
    ...
    var newRow =
        `
        <div class="row_line">
            <p class="third d-inline-block text-center mb-0">` + weight + `мл</p>
            <p class="third third-center d-inline-block text-center mb-0">` + price + 'р'+`</p>
            <div class="third d-inline-block text-center">
                <p class="remove_circle d-inline-block mb-0">×</p>
            </div>
...
function getHtmlElementFromProduct(product)
{
    ...
    let text =
        `<div class="col-sm-12 col-md-4 col-lg-3 mycard" data-product-id="` + product['id'] + `">
            <div class="product_card position-relative">
                 <img class="product_img mt-3" src="/images/products/` + product['img_name'] + `">
                 <h4 class="text-center product_name pl-3 pr-3 mt-3 mb-3">` + product['name'] + `</h4>
                 <p class="product_desc pl-2 pr-2 text-left">` + product['description'] + `</p>
                 <p class="count_p position-absolute mb-2 ml-3"><span class="count_span">248</span> заказов</p>
                 <p class="price position-absolute mb-2 mr-3">` +  price + `</p>
            </div>
        </div>
            `;
...
function getWeightHtmlElement(weight)
{
    var newRow =
        `
        <div class="row_line">
            <p class="third d-inline-block text-center mb-0">` + weight['weight'] + `мл</p>
            <p class="third third-center d-inline-block text-center mb-0">` + Math.round( weight['price']) + 'р'+`</p>
            <div class="third d-inline-block text-center">
                <p class="remove_circle d-inline-block mb-0">×</p>
            </div>
        </div>
...
function getAddOrMilkHtmlElement(item)
{
    var newRow =
        `
        <div class="row_line">
            <p class="third d-inline-block text-center mb-0">` + item['text'] + `</p>
            <p class="third third-center d-inline-block text-center mb-0">` + Math.round( item['price']) + `р</p>
            <div class="third d-inline-block text-center">
                <p class="remove_circle d-inline-block mb-0">×</p>
            </div> 
        </div>
...
{
    let active_link = $('#ul_sub_links').find('.active').first();
    let selected_num = active_link.attr('data-tab-num');

    let img_cafe = $('#categ_my_cafe').find('img').first();
    let img_hot = $('#hot_drinks_cafe').find('img').first();
    let img_cold = $('#cold_drinks_cafe').find('img').first();
    let img_desert = $('#deserts_cafe').find('img').first();

    let name_cafe = $('#categ_my_cafe').find('.div_categ_name').first();
    let name_hot = $('#hot_drinks_cafe').find('.div_categ_name').first();
    let name_cold = $('#cold_drinks_cafe').find('.div_categ_name').first();
    let name_desert = $('#deserts_cafe').find('.div_categ_name').first();
